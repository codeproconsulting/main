...
import { Menu, MenuItem } from "~/immigration/components/ui/navbar-menu";
import { immigrationServices } from "~/immigration/lib/services";
import { navbarDestinations } from "~/immigration/lib/destinations";
import { LOGO_URL } from "~/immigration/lib/site";
import { ServicesDropdown } from "~/immigration/components/ui/ServicesDropdown";
import { DestinationsDropdown } from "~/immigration/components/ui/DestinationsDropdown";

const WHATSAPP_NUMBER = "923701902128";
...
export function Navbar({ className }: { className?: string }) {
  ...
  const isContactPage = location.pathname === "/immigration/contact";
  ...
  return (
    <>
      <div
        className="sticky top-0 z-[100] w-full bg-white/80 backdrop-blur-md border-b border-slate-200/80"
      ...
      >
        <div className={cn("h-20 w-full", className)}>
          <div className="relative w-full max-w-7xl mx-auto h-full flex items-center px-3 sm:px-5 md:px-6 lg:px-4">
            ...
            >
              <img src={LOGO_URL} alt="ProConsulting Immigration" className="h-14 w-auto object-contain" />
            </Link>

            <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-x-6">
              <Link
                to="/"
                className="font-medium text-neutral-700 hover:text-black whitespace-nowrap text-sm lg:text-base py-2"
                onMouseEnter={() => setActive(null)}
              >
                Home
              </Link>
        ...
        {active != null && (
          <div
            ref={dropdownRef}
            className="fixed left-0 right-0 top-20 w-full min-h-[280px] bg-[#FAFAFA] border-t border-slate-200/80 shadow-xl rounded-b-2xl px-8 md:px-12 lg:px-16 py-8 z-[99]"
            onMouseLeave={() => setActive(null)}
          >
            {active === "Destinations" && <DestinationsDropdown />}
            {active === "Services" && <ServicesDropdown />}
          </div>
        )}
      </div>
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <Link to="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center">
                <img src={LOGO_URL} alt="ProConsulting Immigration" className="h-11 w-auto object-contain" />
              </Link>
              <button
                onClick={() => setMobileMenuOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              <div className="space-y-4">
                ...
                {navbarDestinations.map((d) => (
                  <MobileLink key={d.id} to={d.link} setOpen={setMobileMenuOpen}>
                    {d.name}
                  </MobileLink>
                ))}
                <MobileLink to="/immigration/destinations" setOpen={setMobileMenuOpen}>
              ...
              <div className="space-y-4">
                ...
                {immigrationServices.map((s) => (
                  <MobileLink key={s.id} to={s.link} setOpen={setMobileMenuOpen}>
                    {s.title}
                  </MobileLink>
                ))}
                <MobileLink to="/immigration/services" setOpen={setMobileMenuOpen}>
                  View all services
                </MobileLink>
              </div>
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Company</h3>
                <MobileLink to="/" setOpen={setMobileMenuOpen}>
                  Home
                </MobileLink>
                <MobileLink to="/immigration/blog" setOpen={setMobileMenuOpen}>
                  Blog
                </MobileLink>
                <MobileLink to="/immigration/about" setOpen={setMobileMenuOpen}>
                  About Us
                </MobileLink>
              </div>
            </div>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50/50">
              <Link to="/immigration/contact" onClick={() => setMobileMenuOpen(false)}>
...
function MobileLink({
  ...
  return (
    <Link
      to={to}
      className="flex items-center justify-between text-lg font-medium text-black hover:text-pink-600 transition-colors group"
