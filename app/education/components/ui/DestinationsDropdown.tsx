...
function DestinationRow({ name, tag, link, flagIcon }: { name: string; tag: string; link: string; flagIcon: string }) {
  return (
    <Link
      to={link}
    ...
    >
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 group-hover:bg-[#FF4D6D]/10 transition-colors">
        <img src={flagIcon} alt="" className="w-5 h-4 object-cover rounded" />
      </span>
      <div className="min-w-0 flex-1">
        <span className="block text-lg font-bold text-[#0B1B3A] group-hover:text-[#FF4D6D] transition-colors">{name}</span>
    ...
    <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-10 md:gap-12 lg:gap-16 py-2">
      <div className="flex-1 min-w-0">
        <div className="grid grid-cols-2 gap-2">
          {navbarDestinations.map((d) => (
            <DestinationRow key={d.id} name={d.name} tag={d.tag} link={d.link} flagIcon={d.flagIcon} />
          ))}
          <Link
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <span className="block text-lg font-bold text-[#0B1B3A] group-hover:text-[#FF4D6D] transition-colors">View all destinations</span>
      ...
      <div className="flex flex-col gap-4 w-full md:w-[280px] shrink-0">
        <div className="rounded-2xl overflow-hidden shadow-lg text-white p-5 bg-[#0B1B3A]">
          ...
          <p className="text-sm text-white/80 mb-4">Get personalized study abroad guidance</p>
          <Link to="/education/contact" className="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl font-bold text-sm bg-[#FF4D6D] hover:bg-[#E11D48] transition-colors">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-3">
            <a href="tel:+92516135834" className="flex items-center gap-2 text-sm text-slate-700 hover:text-[#FF4D6D] transition-colors">
              <Phone className="h-4 w-4 shrink-0 text-white" />
              PK: (051) 6135834
            </a>
            <a href="tel:+923701902128" className="flex items-center gap-2 text-sm text-slate-700 hover:text-[#FF4D6D] transition-colors">
              <Phone className="h-4 w-4 shrink-0 text-white" />
              PK: +92 370 1902128
            </a>
            <a href="tel:+447432406993" className="flex items-center gap-2 text-sm text-slate-700 hover:text-[#FF4D6D] transition-colors">
              <Phone className="h-4 w-4 shrink-0 text-white" />
              UK: +44 7432 406993
            </a>
          </div>
